...
import { AwsPGPool } from "../../../../pg/lib";
import { AwsMySQLPool } from "../../../../mysql/lib";
...
async function createAwsPGPool(plugins: string = "failover"): Promise<AwsPGPool> {
  const env = await TestEnvironment.getCurrent();
  const props = {
    user: env.databaseInfo.username,
    host: env.databaseInfo.instances[0].host,
  ...
  return new AwsPGPool(props, poolConfig);
}

async function createAwsPGPoolWithICP(plugins: string = "failover"): Promise<AwsPGPool> {
  const env = await TestEnvironment.getCurrent();
  const poolConfig = new AwsPoolConfig({
    maxConnections: 10,
    maxIdleConnections: 3,
...
async function createAwsMySQLPool(plugins: string = "failover"): Promise<AwsMySQLPool> {
  const env = await TestEnvironment.getCurrent();
  const props = {
    user: env.databaseInfo.username,
  ...
  return new AwsMySQLPool(props, poolConfig);
}

async function createAwsMySQLPoolWithICP(plugins: string = "failover"): Promise<AwsMySQLPool> {
  const env = await TestEnvironment.getCurrent();
  const poolConfig = new AwsPoolConfig({
    maxConnections: 10,
    maxIdleConnections: 3,

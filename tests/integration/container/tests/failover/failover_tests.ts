...
export interface FailoverTestOptions {
  plugins: string;
  getExtraConfig?: () => Record<string, any>;
}

export function createFailoverTests(options: FailoverTestOptions) {
  const itIf =
  ...
  return () => {
    ...
    itIf(
      ...
      async () => {
        ...
        await DriverHelper.executeQuery(env.engine, client, "START TRANSACTION");

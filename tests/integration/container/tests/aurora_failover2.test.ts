...
async function initConfigWithReadWriteSplitting(host: string, port: number, connectToProxy: boolean): Promise<any> {
  let config: any = {
    user: env.databaseInfo.username,
    host: host,
    database: env.databaseInfo.defaultDbName,
...
describe("sophia aurora failover2", () => {
  beforeEach(async () => {
  ...
  itIfMinThreeInstance(
    "rw splitting failover reader to writer set read only true false",
    async () => {
      // Connect to writer instance.
      const writerConfig = await initConfigWithReadWriteSplitting(
      ...
      // Assert that we are currently connected to a reader.
  ...
  itIfMinThreeInstance(
    "rw splitting failover to new writer set read only true false",
    async () => {
      // Connect to writer instance.
      const writerConfig = await initConfigWithReadWriteSplitting(
        env.proxyDatabaseInfo.writerInstanceEndpoint,
      ...
      // Assert that we are currently connected to a new writer instance.
      ...
      // Assert that we are currently connected to the new writer instance.
  ...
  itIfMinThreeInstance(
    ...
    async () => {
      ...
      // Assert that we are currently connected to the other reader instance.
  ...
  itIf(
    ...
    async () => {
      const config = await initConfigWithReadWriteSplitting(env.databaseInfo.writerInstanceEndpoint, env.databaseInfo.instanceEndpointPort, false);
  ...
  itIf(
    ...
    async () => {
      const config = await initConfigWithReadWriteSplitting(
        env.proxyDatabaseInfo.writerInstanceEndpoint,
        env.proxyDatabaseInfo.instanceEndpointPort,
        true
      );

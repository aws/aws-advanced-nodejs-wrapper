...
The Okta Authentication Plugin authenticates with a generated token, which Aurora MySQL asks the client to send using the
...
`okta` with MySQL** — the wrapper then enables the option for you, and the token is only ever

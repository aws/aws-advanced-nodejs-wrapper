...
export class MySQLClientWrapper implements ClientWrapper {
  ...
  query(sql: any): Promise<any> {
    const queryTimeout = this.properties.get(WrapperProperties.WRAPPER_QUERY_TIMEOUT.name);
    if (queryTimeout && !sql[this.driverDialect.queryTimeoutPropertyName]) {
      sql[this.driverDialect.queryTimeoutPropertyName] = queryTimeout;
    }

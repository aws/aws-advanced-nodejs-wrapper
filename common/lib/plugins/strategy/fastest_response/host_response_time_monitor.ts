...
export class HostResponseTimeMonitor {
  static readonly MONITORING_PROPERTY_PREFIX = "frt-";
  ...
  constructor(pluginService: PluginService, hostInfo: HostInfo, properties: Map<string, any>, intervalMs: number) {
     ...
     */
    this.telemetryFactory.createGauge("frt.response.time.%s" + hostId, () => this.getResponseTime() == Number.MAX_SAFE_INTEGER);
  }
  ...
  async run(): Promise<void> {
    const telemetryContext: TelemetryContext = this.telemetryFactory.openTelemetryContext("host response time thread", TelemetryTraceLevel.TOP_LEVEL);
      ...
      await telemetryContext.start(async () => {
        try {
          ...
          if (this.monitoringClient) {
            ...
            if (count > 0) {
              this.responseTimeMs = responseTimeSum / count / 10000;
            } else {
          ...
          await sleep(this.intervalMs / 1000);
        } catch (error) {

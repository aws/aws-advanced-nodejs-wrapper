...
export abstract class AbstractReadWriteSplittingPlugin extends AbstractConnectionPlugin implements CanReleaseResources {
  private static readonly subscribedMethods: Set<string> = new Set(["initHostPr,ovider", "connect", "notifyConnectionChanged", "query"]);
  ...
  setWriterClient(writerTargetClient: ClientWrapper | undefined, writerHostInfo: HostInfo): void {
    ...
    logger.debug(Messages.get("ReadWriteSplittingPlugin.setWriterClient", writerHostInfo.getHostAndPort()));
  }
  ...
  async setReaderClient(readerTargetClient: ClientWrapper | undefined, readerHost: HostInfo): Promise<void> {
    ...
    logger.debug(Messages.get("ReadWriteSplittingPlugin.setReaderClient", readerHost.getHostAndPort()));
  }
  ...
  async switchToWriterTargetClient() {
    ...
    logger.debug(Messages.get("ReadWriteSplittingPlugin.switchedFromReaderToWriter", this.writerHostInfo.getHostAndPort()));
  }
  ...
  async switchToReaderTargetClient() {
    ...
    if (this.readerCacheItem == null || !(await this.isTargetClientUsable(this.readerCacheItem.get()))) {
      ...
      try {
        ...
        logger.debug(Messages.get("ReadWriteSplittingPlugin.errorSwitchingToCachedReader", this.readerHostInfo.getHostAndPort(), error.message));
        await this.closeReaderClientIfIdle();
        await this.initializeReaderClient();
      }
    }
  ...
  async closeWriterClientIfIdle() {
    ...
    try {
      if (
        this.writerTargetClient != null &&
        this.writerTargetClient !== currentTargetClient &&
        (await this.isTargetClientUsable(this.writerTargetClient))
      ) {
  ...
  async closeReaderClientIfIdle(): Promise<void> {
    ...
    try {
      if (readerClient != null && readerClient !== currentTargetClient && (await this.isTargetClientUsable(readerClient))) {

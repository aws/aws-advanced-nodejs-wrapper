...
export abstract class AbstractReadWriteSplittingPlugin extends AbstractConnectionPlugin implements CanReleaseResources {
  ...
  protected readerCacheItem?: CacheItem<ClientWrapper> = undefined;
  ...
  async switchCurrentTargetClientTo(newTargetClient: ClientWrapper | undefined, newClientHost: HostInfo | undefined) {
    if (!newTargetClient || !newClientHost) {
      return;
    }
  ...
  async switchToWriterTargetClient() {
    ...
    logger.debug(Messages.get("ReadWriteSplittingPlugin.switchedFromReaderToWriter", this.writerHostInfo?.hostAndPort ?? "unknown"));
  }
      ...
      try {
        ...
        logger.debug(
          Messages.get(
            ...
            this.readerHostInfo?.hostAndPort ?? "undefined reader host",
            error.message
          )
        );
        await this.closeReaderClientIfIdle();
  ...
  async closeReaderClientIfIdle(): Promise<void> {
    ...
    this.readerCacheItem = undefined;

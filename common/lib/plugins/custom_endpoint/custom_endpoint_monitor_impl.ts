...
import { CustomEndpointMonitor } from "./custom_endpoint_monitor";
...
export class CustomEndpointMonitorImpl implements CustomEndpointMonitor {
  ...
  protected readonly customEndpointInfoCache: CacheMap<string, CustomEndpointInfo> = new CacheMap<string, CustomEndpointInfo>();
      ...
      try {
        ...
        if (endpoints.length !== 1) {
          ...
          await new Promise((resolve) => {
            this.timerA = setTimeout(resolve, this.refreshRateMs);
          });
          continue;
        }

        const endpointInfo = CustomEndpointInfo.fromDbClusterEndpoint(endpoints[0]);
        const cachedEndpointInfo = this.customEndpointInfoCache.get(this.customEndpointHostInfo.host);
        ...
        if (cachedEndpointInfo && cachedEndpointInfo.equals(endpointInfo)) {
          ...
          await new Promise((resolve) => {
            this.timerB = setTimeout(resolve, sleepDuration);
          });
          continue;
        }
        ...
        const sleepDuration = Math.max(0, this.refreshRateMs - elapsedTime);
        await new Promise((resolve) => {
          this.timerC = setTimeout(resolve, sleepDuration);
        });
      } catch (e: any) {
        logger.error(Messages.get("CustomEndpointMonitorImpl.exception", this.customEndpointHostInfo.host, e.message));
        throw e;
      }
    }
  }
